...
app = Flask(__name__)
app.secret_key = "turnier-app-dev-secret"

DB_PATH = Path("database") / "tournament.db"
ADMIN_PASSWORD = "1234"
...
def create_groups():
    ...
    for i in range(num_groups):
        group_sizes.append(base_size + (1 if i < remainder else 0))
    ...
    for next_match in next_matches:
        ...
        if refreshed["winner_id"] is None:
            if refreshed["player1_id"] and not refreshed["player2_id"]:
                cursor.execute("""
                    UPDATE knockout_matches
                    SET winner_id = ?, status = 'abgeschlossen'
                    WHERE id = ?
                """, (refreshed["player1_id"], next_match["id"]))
                conn.commit()
                ...
                cursor.execute("""
                    ...
                    SET winner_id = ?, status = 'abgeschlossen'
                    WHERE id = ?
                """, (refreshed["player2_id"], next_match["id"]))
                conn.commit()
                propagate_knockout_winner(conn, next_match["id"])
    ...
    for discipline in disciplines:
        ...
        for i in range(0, len(slots), 2):
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                discipline, current_round_name, round_order, match_number,
                p1, p2, p1_label, p2_label, winner_id, status
            ))
        ...
        while current_size > 2:
            ...
            for i in range(0, len(current_round_ids), 2):
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'wartet')
                """, (
                    discipline, next_round_name, round_order, pair_counter,
                    f"Gewinner {prev_round_name} {i + 1}",
                    f"Gewinner {prev_round_name} {i + 2}",
                    source1, source2
                ))
        ...
        cursor.execute("""
            ...
            WHERE discipline = ? AND winner_id IS NOT NULL
            ORDER BY id
        """, (discipline,))
...
def compute_discipline_results():
    ...
    for discipline in disciplines:
        cursor.execute("""
            ...
            FROM knockout_matches
            WHERE discipline = ?
            ORDER BY round_order DESC, match_order ASC
            LIMIT 1
        """, (discipline,))
        ...
        if final_match["player1_id"] == winner_id:
            second_id = final_match["player2_id"]
        else:
            second_id = final_match["player1_id"]

        winner_name = get_player_name(conn, winner_id)
        second_name = get_player_name(conn, second_id) if second_id else "Freilos"
...
def groups_overview():
    ...
    groups = cursor.fetchall()
    cursor.execute("""
        SELECT group_name, COUNT(*) AS player_count
        FROM groups
        GROUP BY group_name
        ORDER BY group_name
    """)
    ...
    if request.method == "POST":
        ...
        cursor.execute("""
            INSERT INTO tournament_settings (
                ...
                billiard_target, billiard_sets,
                dart_target, dart_sets,
                kicker_target, kicker_sets,
                table_tennis_target, table_tennis_sets,
                cornhole_target, cornhole_sets,
                archery_target, archery_sets
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            group_size, billiard, dart, kicker, table_tennis, cornhole, archery,
...
def update_match(match_id):
    ...
    if request.method == "POST":
        ...
        ids = cursor.fetchone()
        winner_id = None
        if result["winner_side"] == 1:
            winner_id = ids["player1_id"]
        elif result["winner_side"] == 2:
            winner_id = ids["player2_id"]

        cursor.execute("""
            UPDATE matches
            SET score1 = ?, score2 = ?, sets_won_1 = ?, sets_won_2 = ?,
                winner_id = ?, status = 'abgeschlossen'
            WHERE id = ?
        """, (
            final_score1, final_score2,
            result["sets_won_1"], result["sets_won_2"],
            winner_id, match_id
        ))
    ...
    return render_template(
        ...
        match=match,
        settings=settings,
        set_rows=range(1, settings["sets"] + 1),
...
def update_knockout_match(match_id):
    ...
    cursor.execute("""
        SELECT
            ...
            k.discipline,
            k.round_name,
            k.player1_label AS player1_name,
            k.player2_label AS player2_name,
            k.player1_id,
            k.player2_id,
            k.score1,
            k.score2,
